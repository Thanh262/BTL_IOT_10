...
String myDeploymentID = "AKfycbxn8VvkSOGFotxU3wQLR7guZ6bnCSdpybzHPZDiWKIynM3uXCCGYhW3w7Y7Puh13jwWBg";
...
void SendCapturedPhotos() {
  ...
  if (client.connect(host, 443)) {
    ...
    fb = esp_camera_fb_get();
    if(!fb) {
      Serial.println("Khởi động cam thất bại");
      Serial.println("Khởi động lại ESP32 CAM.");
      delay(1000);
      ESP.restart();
      return;
    ...
    Serial.println();
    Serial.println("Đang gửi ảnh đến gg drive.");
...
void setup() {
  ...
  Serial.println();
  Serial.println("Set esp32 wifi sang chế độ trạm.");
  WiFi.mode(WIFI_STA);
  ...
  Serial.println();
  Serial.println("Config ESP32 CAM...");
  ...
  if (err != ESP_OK) {
    Serial.printf("Khởi tạo cam thất bại 0x%x", err);
    Serial.println();
    Serial.println("Khởi động lại ESP32 CAM.");
    delay(1000);
    ESP.restart();
  ...
  Serial.println("Cài đặt cam thành công.");
  Serial.println();
  ...
  Serial.println();
  Serial.println("ESP32-CAM chụp và gửi ảnh đến server mỗi 30 giây.");
  Serial.println();
  delay(2000);
}
//________________________________________________________________________________ 
